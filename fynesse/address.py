...
def GLM_predict(frame, fit_intercept=True, print_coefs=False):
  ...
  intercept = fitted.intercept_[0] if fit_intercept else 0
  if print_coefs:
    print(*(list(fitted.coef_[0])+[intercept]))
  predictions = intercept * np.ones(len(frame.index))
